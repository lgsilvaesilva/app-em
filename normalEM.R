normalEM <- function (x, lambda = NULL, mu = NULL, sigma = NULL, k = 2, mean.constr = NULL, 
  ...
  if (fast == TRUE && k == 2 && arbmean == TRUE) {
    ...
    while (notdone) {
      ...
      while (iter < maxit-1) {
        ...
        iter <- iter + 1
        if (verb) {
          esti.iter[[iter]] <- c(w1=lambda[1],w2=lambda[2], mu1=mu[1],mu2=mu[2], sigma1=sigma[1],sigma2=sigma[2])
        }
      }
    }
    if (iter == maxit) {
    ...
    if (arbmean == FALSE) {
      ...
      a = list(x = x, lambda = lambda[scale.order], mu = mu, 
               ...
               restarts = restarts, ft = "normalmixEM",esti.iter)
    }
    else {
      colnames(postprobs) <- c(paste("comp", ".", 1:k, 
                                     sep = ""))
